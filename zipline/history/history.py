...
class HistorySpec(object):
    ...
    @classmethod
    def spec_key(cls, bar_count, freq_str, field, dividend_adjusted, ffill):
        """
        Used as a hash/key value for the HistorySpec.
        """
        return "{0}:{1}:{2}:{3}:{4}".format(
            bar_count, freq_str, field, dividend_adjusted, ffill)

    def __init__(self, bar_count, frequency, field, dividend_adjusted, ffill,
        ...
        self.key_str = self.spec_key(
            bar_count, frequency.freq_str, field, dividend_adjusted, ffill)
